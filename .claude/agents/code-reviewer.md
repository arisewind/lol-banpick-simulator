...
This project is the **LoL Ban/Pick Simulator** (Electron + React + TypeScript + TailwindCSS, React Context state). In addition to the generic checklist above, read [CLAUDE.md](../../CLAUDE.md) and apply its project conventions, in particular:

- **TailwindCSS / Styling gotchas** — custom `boxShadow` keys do NOT support `/N` opacity; a color-only shadow utility (`shadow-{color}`) needs a paired size class or it emits no shadow; `@import` must precede `@tailwind` in `globals.css`.
- **BP flow invariant** — the current action is always derived from `useBP().getCurrentPhase()`; components must not track "whose turn" separately. Progress totals should derive from `BP_PHASES.length`, not hardcoded literals.
- **i18n** — interface strings go through `t('key')`; flag refactors that replace `t()` calls with hardcoded literals as localization regressions.
- **State management** — three nested Contexts (HeroContext → BPContext → DataContext); preserve provider order and the `banHero`/`pickHero` duplicate-detection invariant.
- **pnpm** (not npm/yarn); verification gate `pnpm lint` + `pnpm tsc --noEmit` + `pnpm test`.
